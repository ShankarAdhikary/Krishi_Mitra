...
CLASSES = ("no_action", "monitor", "irrigate_soon", "irrigate_now")
STATE_NORMALIZATION = {
    "Orissa": "Odisha",
...
def seed(count: int, districts_file: Path | None, history_days: int) -> dict[str, int]:
    ...
    try:
        ...
        for index in range(count):
            ...
            latitude = float(location["latitude"]) + rng.uniform(-0.08, 0.08)
            longitude = float(location["longitude"]) + rng.uniform(-0.08, 0.08)
        ...
        for index, plot in enumerate(plots):
            ...
            for day_offset in range(14):
                ...
                advisory_class = CLASSES[(index + day_offset // 3) % len(CLASSES)]
