...
class GeeOpenMeteoProvider:
    ...
    def fetch_cycle(self, plot: Plot) -> list[SatelliteData]:
        ...
        return self._fetch_earth_engine(plot, latitude, longitude) + self._fetch_weather(
    ...
    def _fetch_earth_engine(
        ...
        if not service_account:
            try:
                service_account = json.loads(Path(self.credentials_path).read_text())["client_email"]
            except (OSError, KeyError, json.JSONDecodeError) as exc:
                raise RuntimeError(
                    "The GEE credentials file must contain a client_email, or set GEE_SERVICE_ACCOUNT"
                ) from exc
        credentials = ee.ServiceAccountCredentials(service_account, self.credentials_path)
        ...
        for index in range(optical_count):
            image = ee.Image(optical.get(index))
            metadata = image.get("system:time_start").getInfo()
            if metadata is None:
                continue
            values = image.reduceRegion(
                reducer=ee.Reducer.median(), geometry=region, scale=10, maxPixels=1_000_000
            ).getInfo()
        ...
        for index in range(radar_count):
            image = ee.Image(radar.get(index))
            metadata = image.get("system:time_start").getInfo()
            if metadata is None:
                continue
            values = image.reduceRegion(
    ...
    @staticmethod
    def _record(
        plot: Plot, source: str, data_type: str, value: object, timestamp: object, satellite_id: str
    ) -> SatelliteData:
        observation_date = datetime.fromtimestamp(
            float(timestamp) / 1000, tz=timezone.utc
        ) if not isinstance(timestamp, datetime) else timestamp
